...
//	}
	...
	static List<String> retrieveMostFrequentlyUsedWords(String helpText, List<String> wordsToExclude) {
		String a = helpText.replace("'", " ");
		String b = a.replace(".", " ");

		String[] result = Arrays.stream(b.split("\\s+")).map(String::toUpperCase).toArray(String[]::new);
		List<String> expectedCountriesList = Arrays.asList(result);
		expectedCountriesList.replaceAll(String::toUpperCase);
		wordsToExclude.replaceAll(String::toUpperCase);
		List<String> unavailable = expectedCountriesList.stream().filter(e -> !wordsToExclude.contains(e))
				.collect(Collectors.toList());

		Map<String, Long> counts = unavailable.stream().collect(Collectors.groupingBy(e -> e, Collectors.counting()));
		...
		return result1;
// WRITE YOUR CODE HERE
//		String x= helpText.toLowerCase().replace("."," ").replace("'"," ");
//        List<String> xx = Arrays.asList(x.split(" "));
//        Map<String, Integer> map = new HashMap<>();
//
//        //create map , Key = text , value = repeat count
//        for(String item :xx){//is in map
//            if (map.containsKey(item)){
//                map.put(item, map.get(item) + 1);
//            }
//            else {//not in map
//                if (!wordsToExclude.contains(item)) {
//                    map.put(item, 1);
//                }
//            }
//        }
...
//                );
//
//        List<String> rtStr= new ArrayList<>();
//        //iterate on map
//        for (Map.Entry<String,Integer> entry : map.entrySet())
//        {
//            if(entry.getValue()==maxEntry.get().getValue()){
//                rtStr.add(entry.getKey());
//            }
//        }
//
//        return  rtStr;
	...
	public static List<String> getFr(String liststr, List<String> get) {
		List<String> result = new ArrayList<>();
		Map<String, Integer> hshmap = new HashMap<String, Integer>();
		String[] splitted = liststr.split(" ");
...
//		 Comparator a = Comparator.comparing((String x, String y)-> x.compareTo(y));
////		 List<String> b = logLines.stream().sorted(a).collect(Collectors.toList());
////		 Collections.sort(logLines);
//			return logLines;
		// WRITE YOUR CODE HERE
		List<String> reorderLines1 = new ArrayList<>();
		for (String str : logLines) {
			if (str.matches("\\A\\p{ASCII}*\\z"))
				reorderLines1.add(str);
		}
		Collections.sort(reorderLines1);
//			reorderLines1.forEach(x->System.out.println(x));
		return reorderLines1;
	}

	public static void main(String[] args) {
//		int[] arr = { 2, 3, 4, 5, 6 };
////		System.out.println(cellCompete(arr, 1));// 4000000
...
//		orderList();
	...
	public static void orderList() {
		...
		Comparator<String> comp = Comparator.comparing(String::new, (x, y) -> x.compareTo(y));
		...
		Comparator<String> comp2 = Comparator.comparing(String::length);
		...
		Test test1 = new Test("h");
		Test test2 = new Test("jimm");
		Test test3 = new Test("mar");
		listTest.add(test);
		listTest.add(test1);
		listTest.add(test2);
		listTest.add(test3);
...
//		Comparator<Test> compT = Comparator.comparing(Test::getName, (x, y)-> x.compareTo(y));
//		Collections.sort(listTest, compT);
//		System.out.println(listTest);

		Comparator<Test> compT2 = Comparator.comparing(Test::getName, (x, y) -> x.length() < y.length() ? -1 : 0);
		Collections.sort(listTest, compT2);
		System.out.println(listTest);
	}
